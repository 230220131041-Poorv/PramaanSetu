...
export default function StudentProfile() {
  ...
  const { user, logout, updateProfile, refreshProfile, isLoading } = useAuth();
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof StudentProfileFormData, string>>>({});
  ...
  const handlePhotoUpload = async (photoUri: string) => {
    ...
    try {
      const result = await userService.uploadProfilePhoto(user.id, photoUri);
      if (result.success && result.url) {
        setFormData(prev => ({ ...prev, avatar_url: result.url || '' }));
        // Refresh profile data from database to get latest avatar_url
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.quickStats}>
          <View style={styles.statItem}>
            <Text style={styles.statValue}>{user?.cgpa || '-'}</Text>
            <Text style={styles.statLabel}>CGPA</Text>
          </View>
          <View style={styles.statDivider} />
          <View style={styles.statItem}>
            <Text style={styles.statValue}>{user?.semester || '-'}</Text>
        ...
        <SafeAreaView style={styles.modalContainer}>
          ...
          <ScrollView style={styles.modalContent} showsVerticalScrollIndicator={false}>
            ...
            <FormInput
              ...
              onChangeText={(text) => setFormData(prev => ({ ...prev, semester: parseInt(text) || 0 }))}
              placeholder="1-8"
            ...
            <FormInput
              ...
              onChangeText={(text) => setFormData(prev => ({ ...prev, cgpa: parseFloat(text) || 0 }))}
